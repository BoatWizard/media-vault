  ...
  return (
    <Link to={`/item/${item.id}`} className="card group hover:border-ink-600 transition-colors block">
      ...
      <div className="p-3">
        ...
        {item.completeness && (
          <p className="text-chrome-dim text-xs font-mono mt-0.5">{item.completeness.replace(/_/g, ' ')}</p>
        )}
      </div>
    </Link>
  )
...
export default function WishlistPage() {
  ...
  const viewingUser = sharedWishlists.find((p) => p.owner_id === viewingOwnerId)
    ...
    <div>
      ...
      {data && data.items.length === 0 && (
        <div className="text-center py-20">
          <p className="font-display text-2xl text-ink-600 tracking-wide">WISHLIST IS EMPTY</p>
          {!viewingOwnerId && (
            <p className="text-chrome-dim text-sm mt-2">
              <Link to="/add?wishlist=true" className="text-acid hover:underline">Add your first wishlist item</Link>
            </p>
          )}
        </div>
      )}
